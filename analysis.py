...
def draw_map(comments):
    try:
        ...
        print(data)
        geo = Geo("《Avengers:Endgame》全国观众地域分布", "数据来源：猫眼电影 数据分析：公众号【Coder不会说】",
...
def draw_bar(comments):
    ...
    bar = Bar('《Avengers:Endgame》观众地域排行榜单', '数据来源：猫眼电影 数据分析：公众号【Coder不会说】', title_pos='center', width=1200, height=600)
...
def draw_cloud(comments):
    ...
    wc = WordCloud(width=1000, height=700, background_color='#000000', font_path='C:\Windows\Fonts\FZYHJW_1.TTF', scale=5,
...
def draw_DateBar(comments):
    ...
    bar = Bar('《Avengers:Endgame》观众评论数量与日期的关系', '数据来源：猫眼电影 数据分析：公众号【Coder不会说】',
...
def draw_TimeBar(comments):
    ...
    bar = Bar('《Avengers:Endgame》观众评论数量与时间的关系', '数据来源：猫眼电影 数据分析：公众号【Coder不会说】',
              title_pos='center', width=1200, height=600)
    attr, value = bar.cast(data)
    bar.add('', attr, value, is_visualmap=True, visual_range=[0, 3500], visual_text_color='#fff', is_more_utils=True,
            is_label_show=True)
